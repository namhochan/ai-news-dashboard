# -*- coding: utf-8 -*-
# modules/style.py
# 전역 스타일 + 퀵 메뉴 (반응형 + 티커 애니메이션 고정)
# v3.7.2 - ticker animation fixed
...
def inject_base_css() -> str:
    ...
    <style>
      ...
      :root{
        ...
        --speed: 25s; --up:#ee6666; --down:#6aa2ff;
      ...
      [data-theme="light"] .quick .card{
        background:#ffffff; border-color:#d9e2f1;
      }
      ...
      [data-theme="light"] .quick a{
        color:#0f172a; background:#f4f7ff; border-color:#cad7f0;
      }
      ...
      /* ===== 칩/뉴스 ===== */
      .chip{display:inline-block;padding:3px 8px;border:1px solid var(--chip);
            border-radius:10px;font-size:12px;margin-right:6px;color:#bfd2ff;}
