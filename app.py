...
THEME_KEYWORDS={
    "AI":["ai","인공지능","오픈ai","엔비디아"],
    "반도체":["반도체","hbm","칩","파운드리"],
    "로봇":["로봇","자율주행","협동로봇"],
    "이차전지":["배터리","전고체","양극재","음극재"],
    "에너지":["전력","정유","태양광","풍력"],
    "조선":["조선","선박","lNG선"],
    "원전":["원전","smr","원자력"],
    "바이오":["바이오","제약","임상"],
...
def detect_themes(news):
    cnt={t:0 for t in THEME_KEYWORDS}
    for n in news:
        txt=(n["title"]+" "+n["desc"]).lower()
        for t,kws in THEME_KEYWORDS.items():
            if any(k in txt for k in kws): cnt[t]+=1
    return sorted([{"theme":t,"count":c} for t,c in cnt.items() if c>0],key=lambda x:x["count"],reverse=True)

all_news=[]; [all_news.extend(fetch_category_news(c,3,100)) for c in CATEGORIES]
...
if not theme_rows: st.info("테마 신호 없음")
    ...
    for tr in top5:
        ...
        picks=[pool[i] for i in rng.choice(len(pool),size=min(4,len(pool)),replace=False)]
        st.markdown(f"### {theme} ({tr['count']}건)")
        cols=st.columns(len(picks))
        for col,(n,t) in zip(cols,picks):
            l,p=fetch_quote(t)
            if not (l and p): continue
            d=(l-p)/p*100; c="red" if d>0 else "blue"
            col.markdown(f"**{n}**<br><span style='color:{c}'>{fmt_number(l,0)} ({fmt_percent(d)})</span>",unsafe_allow_html=True)
