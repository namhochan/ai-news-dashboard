...
# ---------- 사이드바 디버그(선택) ----------
debug = st.sidebar.checkbox("디버그 보기", False)
if debug:
    st.sidebar.write({
        "market_loaded": bool(market),
        "themes_loaded": bool(themes),
        "keyword_map_size": len(keyword_map),
        "headlines_size": len(headlines),
    })
...
if themes:
    for t in themes:
        ...
        st.markdown(f"[관련 뉴스 보기]({t.get('news_link','https://news.google.com/?hl=ko&gl=KR&ceid=KR:ko')})")
        st.divider()
else:
